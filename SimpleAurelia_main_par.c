...
const double PAUSE_FRAC = 0.5;			/* creates a pause after relaxation = PAUSE_FRAC * PERIOD */
...
DEFINE_EXECUTE_AT_END(forces_at_end)
{
	...
	/* Initialize/calc variables on SERIAL or NODE processes  */
		...
		double P_loss_ex = Compute_Power_Loss(tf_ex_end, vel, 0.0);
		double P_loss_sub = Compute_Power_Loss(tf_sub_end, velS, 0.0);
		double P_loss = P_loss_ex + P_loss_sub;
	...
	{ */
		Compute_Averages(vel, fx_tot, 	f_thrust_SMC, 	f_thrust_B, f_drag_SMC, 	f_drag_B,
								P_in, P_loss,	P_thrust_SMC, 	P_thrust_B, P_drag_SMC, 	P_drag_B);
